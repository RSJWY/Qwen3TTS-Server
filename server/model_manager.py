...
class ModelManager:
    ...
    def download_model(self, model_type: str) -> dict[str, str]:
        ...
        t = threading.Thread(
            target=self._download_worker, args=(model_type, cancel_event), daemon=True
        )
        self._download_threads[model_type] = t
        t.start()
        return {"status": "started", "model_type": model_type}

    def _download_worker(self, model_type: str, cancel_event: threading.Event) -> None:
        local_path = os.path.join(self.models_dir, model_type)
        ...
        try:
            ...
            try:
                ...
                logger.info("ModelScope failed: %s, trying HuggingFace", ms_exc)
                if cancel_event.is_set():
                    self._download_status[model_type] = {
                        "status": "cancelled",
                        "progress": 0,
